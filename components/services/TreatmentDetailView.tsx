...
export default function TreatmentDetailView({
  ...
  const isCombinedBeforeAfter =
    combinedGallery.length === 0 && treatment.afterImage === "__combined__";
  const combinedBeforeAfter = isCombinedBeforeAfter
    ? resolveCmsImage(treatment.beforeImage || "", "")
    : "";
  const beforeImg =
    combinedGallery.length === 0 && !isCombinedBeforeAfter
      ? resolveCmsImage(treatment.beforeImage || "", "")
      : "";
  const afterImg =
    combinedGallery.length === 0 && !isCombinedBeforeAfter
      ? resolveCmsImage(treatment.afterImage || "", "")
      : "";
            ...
            <ScrollReveal>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                {combinedGallery.map((src, i) => (
                  <div
                    key={src}
                    className="relative mx-auto aspect-square w-full max-w-md overflow-hidden rounded-2xl shadow-card"
                  >
                    <CmsImage
                      src={src}
                      alt={`${treatment.name} before and after ${i + 1}`}
                      fill
                      className="object-cover"
