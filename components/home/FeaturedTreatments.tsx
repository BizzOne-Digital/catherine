        ...
        <div className="mt-14 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 items-stretch">
          {treatments.map(({ icon: Icon, title, description, href }, i) => (
            <ScrollReveal key={title} delay={i * 0.08} className="h-full">
              <motion.div
                className="group relative flex h-full min-h-[16rem] flex-col rounded-xl border border-gold/25 bg-ivory/95 p-6 shadow-card transition-all duration-500 hover:border-gold/45 hover:shadow-gold-sm"
                whileHover={{ y: -4 }}
              >
                ...
                <h3 className="mb-2 font-playfair text-xl text-text-dark transition-colors duration-300 group-hover:text-gold">
                  {title}
                </h3>

                <p className="mb-6 flex-1 font-inter text-sm leading-relaxed text-soft-taupe">
                  {description}
                </p>

                <Link
                  href={href}
                  className="inline-flex items-center gap-2 font-inter text-xs uppercase tracking-[0.14em] text-gold/80 transition-all duration-300 group-hover:gap-3 group-hover:text-gold"
                >
                  Learn More
                  <ArrowRight size={13} />
                </Link>
